/*=========================================================================
DO FILE NAME:			Main_analyses1_FirstEver.do
...
cap log close
log using $log\firstever_cox.log, replace
*st
strate infection_tv, per(1000) 
strate infection_tv, per(1000) output ($results/strate_first, replace)


label define infection_tv 0 "No infection" 1"Infection" 
label values infection_tv infection_tv


capture file close textfile 
file open textfile using $results/firsteverinfection.csv, write replace
...
log using $log/maintype_cox.log, replace
...
capture file close textfile 
*file close textfile
file open textfile_type using $results/firsteverinfection.csv, write append
...
file write  textfile_type ";"  %3.2f (`hr_ful_6') " (" %3.2f (`lci_ful_6')  "-" %3.2f (`uci_ful_6') ")" _n
...
gen followup_dem=(dementia_date1-start_fup)/365.25 if _d==1 & infection_tv==0
label var followup_dem "Length of CPRD follow up in years in those with dementia but no infections" 

gen followup_inf_dem=(dementia_date1-infection_date1)/365.25 if _d==1 & infection_tv==1
label var followup_inf_dem "Length of CPRD follow up in years in those with infections and dementia" 

sum followup_dem followup_inf_dem, d //mean 4.76, median, 4.14 for no infections, mean 4.30, median 3.70 for infections

codebook followup_dem 
codebook followup_inf_dem
